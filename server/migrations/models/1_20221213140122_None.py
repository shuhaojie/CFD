...
CREATE TABLE IF NOT EXISTS `archive` (
    `uuid` VARCHAR(255) NOT NULL  PRIMARY KEY DEFAULT '903df808-7aab-11ed-9bd2-e0d045dbb4d7',
...
CREATE TABLE IF NOT EXISTS `fluentprof` (
...
CREATE TABLE IF NOT EXISTS `token` (
    `id` INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
    `access_token` VARCHAR(255) NOT NULL  COMMENT '文件md5值',
...
CREATE TABLE IF NOT EXISTS `uknow` (
    ...
    `task_name` VARCHAR(255) NOT NULL  COMMENT '任务名称' DEFAULT '<tortoise.fields.data.CharField object at 0x0000020CFBF90190>',
    `username` VARCHAR(255) NOT NULL  COMMENT '用户名',
    ...
    `icem_params` LONGTEXT NOT NULL  COMMENT 'Icem参数',
    `fluent_hardware_level` VARCHAR(255) NOT NULL  COMMENT 'Fluent硬件配置等级',
    `fluent_params` LONGTEXT NOT NULL  COMMENT 'Fluent参数',
    ...
    `icem_log_file_path` VARCHAR(255)   COMMENT 'icem日志文件路径',
    `icem_result_file_path` VARCHAR(255)   COMMENT 'icem结果路径',
    ...
    `fluent_log_file_path` VARCHAR(255)   COMMENT 'fluent日志文件',
    `fluent_result_file_path` VARCHAR(255)   COMMENT 'fluent结果路径',
